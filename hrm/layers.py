...
class HierarchicalAttention(nn.Module):
    ...
    def __init__(
        ...
        assert self.head_dim * num_heads == low_dim, "low_dim must be divisible by num_heads"
...
class HighLevelModule(nn.Module):
    ...
    def __init__(
        ...
        self.recurrent_layers = nn.ModuleList(
            [
                HighLevelRecurrentLayer(
                    hidden_dim, hidden_dim, dropout, activation
                )
...
class LowLevelRecurrentLayer(nn.Module):
    ...
    def forward(self, x: Tensor, hidden: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        new_hidden = (1 - update) * hidden + update * (syntax * candidate + (1 - syntax) * hidden)
