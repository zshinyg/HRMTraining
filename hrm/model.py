...
class HRMModel(nn.Module):
    ...
    def forward(
        ...
        if high_level_states is None:
            high_level_states = [
                [
                    torch.zeros(
                        batch_size, self.config.high_level_dim, device=device
                    )
        ...
        if low_level_states is None:
            low_level_states = [
                [
                    torch.zeros(
                        batch_size, self.config.low_level_dim, device=device
                    )
        ...
        # Initialize outputs
        logits = torch.zeros(
            batch_size, seq_len, self.config.vocab_size, device=device
        )
        ...
        for step in range(input_ids.size(1), max_length):
            ...
            for batch_idx in range(batch_size):
                for beam_idx in range(num_beams):
                    ...
                    next_input_ids[global_beam_index, :-1] = input_ids[global_source_index]
                    next_attention_mask[global_beam_index, :-1] = attention_mask[global_source_index]
                    ...
                    if token_id == eos_token_id:
                        ...
                        if beam_scores[batch_idx, beam_idx] > best_scores[batch_idx]:
                            ...
                            best_sequences[batch_idx, :next_input_ids.size(1)] = next_input_ids[global_beam_index]
            ...
            if early_stopping and torch.all(finished_sequences.view(batch_size, num_beams)):
                break
    ...
    def _resize_token_embeddings(
        ...
        new_embeddings = nn.Embedding(
            new_num_tokens, old_embeddings.embedding_dim, padding_idx=old_embeddings.padding_idx
        )
