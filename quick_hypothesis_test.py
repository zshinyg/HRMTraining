...
def generate_code(
    model: MockHRMModel, prompt: str, max_tokens: int = 256
) -> Tuple[str, float]:
    """
...
def run_hypothesis_test():
    ...
    baseline_model, baseline_params = create_baseline_model(
    ...
    print(
        f"  Parameter Efficiency: {baseline_params/hrm_params:.1f}x fewer parameters in HRM"
    )

    print("\nAccuracy Metrics:")
    print(f"  HRM Pass@1: {hrm_pass_at_1:.2%} ({hrm_correct}/{n_samples})")
    print(
        f"  Baseline Pass@1: {baseline_pass_at_1:.2%} ({baseline_correct}/{n_samples})"
    )
    ...
    if hrm_pass_at_1 >= baseline_pass_at_1:
        result = "✓ VALIDATED"
        explanation = (
            f"HRM ({hrm_params:.1f}M) outperforms Baseline ({baseline_params:.1f}M)"
        )
    else:
        result = "✗ NOT VALIDATED"
        explanation = (
            f"Baseline ({baseline_params:.1f}M) outperforms HRM ({hrm_params:.1f}M)"
        )
