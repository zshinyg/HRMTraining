...
_CACHE_DIR = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "checkpoints", "tokenizer"
)
...
def encode(
    ...
    if isinstance(padding, bool):
        if padding:
            padding_strategy: Union[bool, str] = (
                "max_length" if max_length else "longest"
            )
        else:
...
def create_training_batch(
    ...
    # Prepare input_ids, labels and attention_mask arrays
    input_ids = []
    labels = []
    attention_masks = []
