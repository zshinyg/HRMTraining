...
REQUIRED_DIRS = ["scripts", "configs", ".github/workflows", "scripts/security"]
...
class QuickCIValidator:
    ...
    def test_github_actions(self):
        ...
        if invalid_workflows:
            result["passed"] = False
            result["message"] = (
                f"Invalid GitHub Actions workflows: {', '.join(invalid_workflows)}"
            )
            self.all_passed = False
    ...
    def test_docker_files(self):
        ...
        if not os.path.isfile(dockerfile_path):
            ...
            with open(dockerfile_path, "r") as f:
                ...
                if (
                    "ENTRYPOINT" not in dockerfile_content
                    and "CMD" not in dockerfile_content
                ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Quick validation for HRM-CodeGen CI/CD pipeline"
    )
    parser.add_argument(
