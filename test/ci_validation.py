...
REQUIRED_PACKAGES = [
    ...
    "transformers", "wandb", "pytest", "docker"
]
...
class CIPipelineValidator:
    ...
    def __init__(self, docker_tests: bool = False, full_tests: bool = False, verbose: bool = False):
        """
    ...
    def test_python_functionality(self) -> Dict[str, Union[bool, str]]:
        ...
        try:
            ...
            # Test subprocess
            output = subprocess.check_output(["python", "-c", "print('subprocess test')"])
        ...
        self._log(f"Python functionality tests: {'PASSED' if result['passed'] else 'FAILED'}")
        return result
    ...
    def test_pass_at_k_components(self) -> Dict[str, Union[bool, str]]:
        ...
        try:
            ...
            try:
                ...
                if self.full_tests:
                    ...
                    assert evaluation_result["num_passed"] > 0, "Pass@k evaluation failed"
                    assert "pass@1" in evaluation_result["metrics"], "Pass@k metrics missing"
        ...
        self._log(f"Pass@k component tests: {'PASSED' if result['passed'] else 'FAILED'}")
        return result
    ...
    def test_docker_integration(self) -> Dict[str, Union[bool, str]]:
        ...
        try:
            ...
            if self.full_tests:
                ...
                try:
                    build_cmd = ["docker", "build", "-t", test_tag, "--target", "base", "."]
                    ...
                    images = subprocess.check_output(["docker", "images", "-q", test_tag]).decode("utf-8")
                    if not images.strip():
                        result["passed"] = False
                        ...
                        # Clean up
                        subprocess.run(["docker", "rmi", test_tag], 
                                      stdout=subprocess.DEVNULL, 
                                      stderr=subprocess.DEVNULL)
                
                except subprocess.CalledProcessError as e:
                    result["passed"] = False
                    result["message"] = f"Docker build failed: {e.stderr.decode('utf-8') if e.stderr else str(e)}"
            else:
        ...
        self._log(f"Docker integration tests: {'PASSED' if result['passed'] else 'FAILED'}")
        return result
    ...
    def test_github_actions(self) -> Dict[str, Union[bool, str]]:
        ...
        try:
            ...
            if missing_workflows:
                result["passed"] = False
                result["message"] = f"Missing GitHub Actions workflows: {', '.join(missing_workflows)}"
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate CI/CD pipeline for HRM-CodeGen")
    parser.add_argument("--docker", action="store_true", help="Run Docker-specific tests")
    parser.add_argument("--full", action="store_true", help="Run all tests including resource-intensive ones")
    parser.add_argument("--verbose", action="store_true", help="Show detailed test output")
    args = parser.parse_args()
    
    # Run validation tests
    validator = CIPipelineValidator(
        docker_tests=args.docker,
        full_tests=args.full,
