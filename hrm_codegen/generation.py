...
def generate_code(
    ...
    **kwargs
    ...
    if isinstance(prompt, str) and not prompt.strip().startswith("#"):
        ...
        prompt = [format_prompt(p) if not p.strip().startswith("#") else p for p in prompt]
    ...
    if stop_tokens:
        if isinstance(generated_text, str):
            ...
            generated_text = [apply_stop_tokens(text, stop_tokens) for text in generated_text]
...
def stream_tokens(
    ...
    **generate_kwargs
    ...
    # Encode prompt
    input_ids = encode(prompt, return_tensors="pt")["input_ids"].to(next(model.parameters()).device)
