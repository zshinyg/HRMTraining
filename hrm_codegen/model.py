...
class HRMCodeGenerator(nn.Module):
    ...
    def initial_carry(self, batch: Dict[str, torch.Tensor]):
        ...
        return HierarchicalReasoningModel_ACTV1Carry(
            ...
            steps=torch.zeros(
                (batch_size,), dtype=torch.int32, device=batch["input_ids"].device
            ),
            halted=torch.ones(
                (batch_size,), dtype=torch.bool, device=batch["input_ids"].device
            ),
    ...
    def _generate_tokens(
        ...
        **kwargs,
        ...
        unfinished_sequences = torch.ones(
            effective_batch_size, dtype=torch.bool, device=device
        )
        ...
        while cur_len < max_length and torch.any(unfinished_sequences):
            ...
            if top_k is not None and top_k > 0:
                indices_to_remove = (
                    torch.topk(next_token_logits, k=top_k, dim=-1)[0][..., -1, None]
                    <= next_token_logits
                )
    ...
    @classmethod
    def from_pretrained(
        cls, model_path: str, config: Optional[Dict[str, Any]] = None
    ) -> "HRMCodeGenerator":
        """
