...
class TestSpecialTokenHandling:
    ...
    def test_no_special_tokens(self):
        ...
        text = "Hello, world!"
        encoded_with_special = encode(text, add_bos=True, add_eos=True, return_tensors="pt")
        encoded_without_special = encode(text, add_bos=False, add_eos=False, return_tensors="pt")
        # The version with special tokens should be longer
        assert encoded_with_special["input_ids"].size(1) >= encoded_without_special["input_ids"].size(1)
...
class TestBatchProcessing:
    ...
    def test_truncation(self):
        ...
        max_length = 50
        encoded = encode(long_text, max_length=max_length, truncation=True, return_tensors="pt")
...
class TestTrainingBatchCreation:
    ...
    def test_create_training_batch(self, sample_prompts, sample_completions):
        ...
        batch = create_training_batch(sample_prompts, sample_completions, return_tensors="pt")
    ...
    def test_label_shifting(self, sample_prompts, sample_completions):
        ...
        batch = create_training_batch([sample_prompts[0]], [sample_completions[0]], return_tensors="pt")
        ...
        for i in range(completion_start, batch["labels"].size(1) - 1):
            if batch["labels"][0, i].item() != -100:  # Skip padding tokens
                assert batch["input_ids"][0, i + 1].item() == batch["labels"][0, i].item()
    ...
    def test_max_length_handling(self, sample_prompts, sample_completions):
        ...
        batch = create_training_batch(sample_prompts, sample_completions, max_length=max_length, return_tensors="pt")
