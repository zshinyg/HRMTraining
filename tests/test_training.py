...
class TestTrainerInitialization:
    ...
    def test_optimizer_creation(self, trainer):
        ...
        # Check learning rate
        warmup_lr = 0.0 if trainer.config.warmup_steps > 0 else trainer.config.learning_rate
...
class TestTrainingStep:
    ...
    def test_optimizer_step(self, trainer):
        ...
        if lr > 0.0:
            assert params_changed, "Parameters did not change after optimizer step with positive LR"
...
class TestCheckpointing:
    ...
    def test_cleanup_checkpoints(self, trainer, temp_dir):
        ...
        checkpoints = [f for f in os.listdir(temp_dir) if f.startswith("step-") and f.endswith(".pt")]
    ...
    def test_save_best_model(self, trainer, temp_dir):
        ...
        should_save = trainer.state.should_save_best("eval_loss", higher_is_better=False)
    ...
    def test_early_stopping(self, trainer, monkeypatch):
        ...
        def train_with_early_stop(*args, **kwargs):
            ...
            original_should_continue = lambda: (
                trainer.state.global_step < trainer.config.max_steps and
                (trainer.config.max_epochs is None or trainer.state.epoch < trainer.config.max_epochs)
            )
            ...
            trainer._should_continue = types.MethodType(original_should_continue, trainer)
