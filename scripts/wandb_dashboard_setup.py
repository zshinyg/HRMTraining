...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--project", type=str, default="hrm-codegen", help="W&B project name"
    )

    parser.add_argument(
        "--run-name", type=str, default="m1-27m-training", help="W&B run name"
    )

    parser.add_argument("--entity", type=str, default=None, help="W&B entity/team name")
...
def extract_thresholds(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "safety" in config:
        if (
            "loss_watchdog" in config["safety"]
            and "threshold" in config["safety"]["loss_watchdog"]
        ):
...
def main() -> None:
    ...
    if not WANDB_AVAILABLE:
        logger.error(
            "W&B not installed. Please install wandb package: pip install wandb"
        )
        sys.exit(1)
    ...
    if not is_wandb_authenticated():
        logger.error(
            "W&B not authenticated. Please run 'wandb login' or set WANDB_API_KEY environment variable"
        )
        sys.exit(1)
