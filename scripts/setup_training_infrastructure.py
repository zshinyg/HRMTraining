...
class InfrastructureSetup:
    ...
    def __init__(
        ...
        logger.info(
            f"Infrastructure setup initialized for experiment: {self.experiment_name}"
        )
    ...
    def validate_environment(self) -> bool:
        ...
        for package_spec in REQUIRED_PACKAGES:
            ...
            try:
                ...
                package_results[package_name] = {
                    ...
                    "version": version,
                    "required": (
                        package_spec.split(package_name)[1]
        ...
        if PSUTIL_AVAILABLE:
            ...
            system_results["note"] = (
                "psutil not available, skipping system resource checks"
            )
        ...
        if WANDB_AVAILABLE and self.config["logging"]["use_wandb"]:
            ...
            if not wandb_results["api_key_set"]:
                logger.warning(
                    "W&B API key not set. Set the WANDB_API_KEY environment variable."
                )
        else:
            wandb_results["available"] = False
            if self.config["logging"]["use_wandb"]:
                logger.warning(
                    "W&B is enabled in config but not installed. Install with: pip install wandb"
                )
        ...
        required_valid = (
            ...
            and system_valid
            and results["data_directory"]["valid"]
            and results["output_directory"]["writable"]
        )
    ...
    def validate_config(self) -> bool:
        ...
        if "model" in self.config:
            ...
            model_missing_keys = [
                key for key in model_required_keys if key not in model_config
            ]
            ...
            if model_missing_keys:
                logger.error(
                    f"Missing required model configuration keys: {', '.join(model_missing_keys)}"
                )
        ...
        if "training" in self.config:
            ...
            training_missing_keys = [
                key for key in training_required_keys if key not in training_config
            ]
            ...
            if training_missing_keys:
                logger.error(
                    f"Missing required training configuration keys: {', '.join(training_missing_keys)}"
                )
            ...
            if "batch_size" in training_config:
                ...
                if not isinstance(batch_size, int) or batch_size <= 0:
                    ...
                    valid = False
                    logger.error(
                        f"Invalid batch size: {batch_size}. Must be a positive integer."
                    )
        ...
        if "data" in self.config:
            ...
            data_missing_keys = [
                key for key in data_required_keys if key not in data_config
            ]
            ...
            if data_missing_keys:
                logger.error(
                    f"Missing required data configuration keys: {', '.join(data_missing_keys)}"
                )
        ...
        if self.config.get("distributed", {}).get("enabled", False):
            ...
            dist_missing_keys = [
                key for key in dist_required_keys if key not in dist_config
            ]
            ...
            if dist_missing_keys:
                logger.error(
                    f"Missing required distributed configuration keys: {', '.join(dist_missing_keys)}"
                )
            ...
            if "world_size" in dist_config:
                ...
                if not isinstance(world_size, int) or world_size <= 0:
                    ...
                    valid = False
                    logger.error(
                        f"Invalid world size: {world_size}. Must be a positive integer."
                    )
    ...
    def create_directories(self) -> bool:
        ...
        try:
            ...
            if not self.data_dir.exists():
                ...
                logger.warning(
                    "Data directory was created but may be empty. Please add required data files."
                )
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        results["directories_created"] = (
            output_dir_exists
            and data_dir_exists
            and results["directories"].get("checkpoint_dir_exists", False)
        )
        ...
        if TORCH_AVAILABLE:
            results["gpu"] = {
                "cuda_available": torch.cuda.is_available(),
                "mps_available": hasattr(torch, "mps")
                and torch.backends.mps.is_available(),
            }
        ...
        results["healthy"] = (
            ...
            and results["directories_created"]
            and len(self.components_failed) == 0
        )
    ...
    def setup(self) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Setting up training infrastructure for experiment: {self.experiment_name}"
        )
        start_time = time.time()
...
def main():
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate, don't initialize"
    )
    parser.add_argument(
        "--initialize-all", action="store_true", help="Initialize all components"
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force initialization even if validation fails",
    )

    # Testing options
    parser.add_argument(
        "--check-gpu", action="store_true", help="Run GPU performance test"
    )
    parser.add_argument(
        "--check-ci", action="store_true", help="Test CI/CD integration"
    )
    ...
    if args.json:
        ...
        print(
            f"Components Initialized: {', '.join(results['components_initialized']) or 'None'}"
        )
