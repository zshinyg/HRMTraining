...
try:
    ...
    print(
        "Make sure you're running this script from the root directory of the project."
    )
    sys.exit(1)
...
class HRMSetupTester:
    ...
    def print_summary(self):
        ...
        for test_name, passed in self.results.items():
            status = (
                ...
                if passed
                else ColoredOutput.error("FAILED")
            )
        ...
        if all_passed:
            print(
                ColoredOutput.success(
                    "All tests passed! Your HRM-CodeGen setup is ready for training."
                )
            )
            print("\nNext steps:")
            print("1. Run data conversion: python scripts/convert_mbpp.py")
            print(
                "2. Start training: python scripts/train.py --config configs/hrm/mbpp_base.yaml"
            )
        else:
            print(
                ColoredOutput.error(
            ...
            print(
                "- Path issues: Make sure you're running from the project root directory"
            )
    ...
    def test_imports(self):
        ...
        try:
            ...
            print(
                ColoredOutput.success("  ✓ All required modules imported successfully")
            )
            ...
            print(
                ColoredOutput.warning(
                    f"    Suggestion: pip install {str(e).split()[-1]}"
                )
            )
    ...
    def test_config_loading(self):
        ...
        try:
            ...
            if os.path.exists(self.config_path):
                ...
                print(
                    ColoredOutput.warning(
                        f"    Suggestion: Create the configs directory and add YAML files"
                    )
                )
    ...
    def test_model_initialization(self):
        ...
        try:
            ...
            print(
                ColoredOutput.success(
                    f"  ✓ Model initialized with {num_params:,} parameters in {init_time:.2f}s"
                )
            )
            ...
            print(
                ColoredOutput.warning(
                    "    Suggestion: Check model architecture and configuration"
                )
            )
    ...
    def test_forward_pass(self):
        ...
        try:
            ...
            if self.model is None:
                print(
                    ColoredOutput.error(
                        "  ✗ Model not initialized, skipping forward pass test"
                    )
                )
            ...
            if logits.shape == expected_shape:
                print(
                    ColoredOutput.success(
                        f"  ✓ Forward pass successful in {forward_time:.4f}s"
                    )
                )
                ...
                if "loss" in outputs_with_labels:
                    ...
                    print(
                        ColoredOutput.success(
                            f"  ✓ Loss computation successful: {loss.item():.4f}"
                        )
                    )
                ...
                print(
                    ColoredOutput.error(
                        f"  ✗ Incorrect output shape: {logits.shape}, expected {expected_shape}"
                    )
                )
                self.results["test_forward_pass"] = False
            ...
            print(
                ColoredOutput.warning(
                    "    Suggestion: Check model implementation and input shapes"
                )
            )
            self.results["test_forward_pass"] = False
    ...
    def test_model_save_load(self):
        ...
        try:
            ...
            if self.model is None:
                print(
                    ColoredOutput.error(
                        "  ✗ Model not initialized, skipping save/load test"
                    )
                )
            ...
            torch.save(
                {
                    ...
                    "config": (
                        self.test_config.to_dict()
                        if hasattr(self.test_config, "to_dict")
                        else asdict(self.test_config)
                    ),
                },
                save_path,
            )
            ...
            if set(original_params.keys()) == set(loaded_params.keys()):
                ...
                if all_equal:
                    print(
                        ColoredOutput.success(
                            "  ✓ Original and loaded parameters match"
                        )
                    )
                    ...
                    print(
                        ColoredOutput.error(
                            "  ✗ Parameter values don't match after loading"
                        )
                    )
                    self.results["test_model_save_load"] = False
            else:
                print(
                    ColoredOutput.error("  ✗ Parameter keys don't match after loading")
                )
                self.results["test_model_save_load"] = False
            ...
            print(
                ColoredOutput.warning(
                    "    Suggestion: Check model serialization and file permissions"
                )
            )
            self.results["test_model_save_load"] = False
    ...
    def test_data_pipeline(self):
        ...
        try:
            ...
            print(
                ColoredOutput.success(
                    f"  ✓ Data pipeline test successful in {pipeline_time:.4f}s"
                )
            )
            print(
                ColoredOutput.success(
                    f"  ✓ Processed {len(dataloader)} batches of size {batch_size}"
                )
            )
            ...
            try:
                ...
                print(
                    ColoredOutput.success("  ✓ MBPPDataset class imported successfully")
                )
                ...
                if os.path.exists(data_dir) and any(
                    ...
                    print(
                        ColoredOutput.info(
                            "    Run: python scripts/convert_mbpp.py to download and process data"
                        )
                    )
                ...
                print(
                    ColoredOutput.warning(
                        "  ⚠ Could not import MBPPDataset, but basic data pipeline works"
                    )
                )
                self.results["test_data_pipeline"] = True
            ...
            print(
                ColoredOutput.warning(
                    "    Suggestion: Check DataLoader and Dataset implementations"
                )
            )
    ...
    def test_generation(self):
        ...
        try:
            ...
            if self.model is None:
                print(
                    ColoredOutput.error(
                        "  ✗ Model not initialized, skipping generation test"
                    )
                )
            ...
            if generated_ids.shape[1] >= expected_min_length:
                print(
                    ColoredOutput.success(
                        f"  ✓ Generation successful in {generation_time:.4f}s"
                    )
                )
                ...
                try:
                    ...
                    print(
                        ColoredOutput.success(
                            f"  ✓ Beam search successful in {beam_time:.4f}s"
                        )
                    )
                    ...
                    print(
                        ColoredOutput.warning(
                            "  ⚠ Beam search not implemented, but basic generation works"
                        )
                    )
                    self.results["test_generation"] = True
            else:
                print(
                    ColoredOutput.error(
                        f"  ✗ Generated sequence too short: {generated_ids.shape[1]}, expected at least {expected_min_length}"
                    )
                )
                self.results["test_generation"] = False
            ...
            print(
                ColoredOutput.warning(
                    "    Suggestion: Check generation logic and parameters"
                )
            )
            self.results["test_generation"] = False
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Test the HRM-CodeGen setup")
