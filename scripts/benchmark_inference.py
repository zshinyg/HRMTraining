...
def benchmark_generation(
    ...
    # Main benchmark loop
    generation_desc = (
        f"{'Beam' if generation_mode == 'beam' else 'Greedy/Sampling'} generation"
    )
    if generation_mode == "beam":
    ...
    # First token latency
    first_token_latency = (
        np.mean(timings["first_token"]) if "first_token" in timings else 0
    )
...
def benchmark_inference(
    ...
    for batch_size in batch_sizes:
            ...
            for mode, mode_results in generation_results.items():
                if mode == "beam":
                    for beam_key, beam_result in mode_results.items():
                        ...
                        for metric_key, metric_value in beam_result[
                            "throughput"
                        ].items():
                            flat_metrics[
                                f"benchmark/generation/{mode}/{beam_key}/{metric_key}_bs{batch_size}"
                            ] = metric_value
                    ...
                    for metric_key, metric_value in mode_results["throughput"].items():
                        flat_metrics[
                            f"benchmark/generation/{mode}/{metric_key}_bs{batch_size}"
                        ] = metric_value
        ...
        logger.info(
            f"  Forward pass: {forward_results['throughput']['tokens_per_second']:.2f} tokens/s"
        )
        for mode, mode_results in generation_results.items():
            if mode == "beam":
                for beam_key, beam_result in mode_results.items():
                    logger.info(
                        f"  {beam_key}: {beam_result['throughput']['tokens_per_second']:.2f} tokens/s"
                    )
            else:
                logger.info(
                    f"  {mode}: {mode_results['throughput']['tokens_per_second']:.2f} tokens/s"
                )
    ...
    if stress_test:
        ...
        for seq_length in long_seq_lengths:
            if seq_length > config.model.max_position_embeddings:
                logger.warning(
                    f"Skipping sequence length {seq_length} as it exceeds model's max position embeddings"
                )
                continue
            ...
            generation_results = benchmark_generation(
                ...
                max_new_tokens=min(
                    512, max_new_tokens
                ),  # Limit generation length for stress test
                generation_params={
                    "temperature": 0.0,
                    "top_p": 1.0,
                    "top_k": 0,
            ...
            if WANDB_AVAILABLE and wandb_run_id:
                flat_metrics = {
                    ...
                    f"stress_test/forward/tokens_per_second": forward_results[
                        "throughput"
                    ]["tokens_per_second"],
                    f"stress_test/generation/tokens_per_second": generation_results[
                        "throughput"
                    ]["tokens_per_second"],
                    f"stress_test/generation/first_token_ms": generation_results[
                        "latency"
                    ]["first_token_ms"],
                    f"stress_test/generation/total_generation_ms": generation_results[
                        "latency"
                    ]["total_generation_ms"],
                }
                ...
                if resource_stats:
                    for key, stats in resource_stats.items():
                        for stat_name, value in stats.items():
                            flat_metrics[f"stress_test/resources/{key}_{stat_name}"] = (
                                value
                            )
            ...
            logger.info(
                f"  Generation: {generation_results['throughput']['tokens_per_second']:.2f} tokens/s"
            )
