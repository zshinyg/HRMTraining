...
class ReliabilityMonitor:
        ...
        if self.is_apple_silicon:
            try:
                if torch.backends.mps.is_available():
                    metrics.mps_memory_used = torch.mps.current_allocated_memory() / (
                        1024**3
                    )  # GB
                    metrics.mps_memory_total = torch.mps.driver_allocated_memory() / (
                        1024**3
                    )  # GB
    ...
    def _collect_training_metrics(
        self, model=None, optimizer=None, loss=None
    ) -> TrainingMetrics:
        """
        ...
        if model is not None:
            ...
            if torch.cuda.is_available():
                ...
                metrics.memory_allocated = torch.mps.current_allocated_memory() / (
                    1024**3
                )  # GB
                metrics.memory_reserved = torch.mps.driver_allocated_memory() / (
                    1024**3
                )  # GB
    ...
    def _check_for_anomalies(self) -> List[Dict[str, Any]]:
        ...
        if (
            self.current_system_metrics.mps_memory_used is not None
            and self.current_system_metrics.mps_memory_total is not None
        ):
            mps_usage = (
                self.current_system_metrics.mps_memory_used
                / self.current_system_metrics.mps_memory_total
            )
        ...
        if len(self.training_metrics_history) >= 2:
            ...
            if (
                self.current_training_metrics.loss is not None
                and self.training_metrics_history[-2].loss is not None
            ):
                loss_ratio = self.current_training_metrics.loss / max(
                    self.training_metrics_history[-2].loss, 1e-8
                )
            ...
            if len(self.training_metrics_history) > THROUGHPUT_WINDOW and all(
                m.throughput is not None
                for m in self.training_metrics_history[-THROUGHPUT_WINDOW:]
            ):
                ...
                past_throughput = np.mean(
                    [
                        ...
                        for m in self.training_metrics_history[
                            -THROUGHPUT_WINDOW : -THROUGHPUT_WINDOW // 2
                        ]
                    ]
                )
        ...
        heartbeat_interval = (
            ...
            .get("heartbeat", {})
            .get("interval_seconds", HEARTBEAT_INTERVAL)
        )
        ...
        while self.is_running:
            try:
                ...
                if len(self.training_metrics_history) > max_history:
                    self.training_metrics_history = self.training_metrics_history[
                        -max_history:
                    ]
                ...
                if self.attached_pid is not None:
                    if not psutil.pid_exists(self.attached_pid):
                        logger.warning(
                            f"Attached process {self.attached_pid} no longer exists"
                        )
                        self.stop()
                        break
    ...
    def benchmark(
        ...
        """
        logger.info(
            f"Starting benchmark with batch_size={batch_size}, seq_len={seq_len}"
        )
        ...
        results = {
            ...
            "throughput": {
                ...
                "tokens_per_second": float(
                    batch_size * seq_len / np.mean(forward_times)
                ),
            },
            "memory": {
                ...
                "bytes_per_token": float(
                    np.mean(memory_usage) / (batch_size * seq_len)
                ),
            },
        }
        ...
        logger.info(
            f"  Throughput: {results['throughput']['samples_per_second']:.2f} samples/sec"
        )
        logger.info(
            f"  Throughput: {results['throughput']['tokens_per_second']:.2f} tokens/sec"
        )
        logger.info(
            f"  Memory per token: {results['memory']['bytes_per_token'] / 1024:.2f} KB"
        )
    ...
    def system_optimization_check(self) -> Dict[str, Any]:
        ...
        if self.config:
            results["checks"]["config"] = {
                "gradient_checkpointing": self.config.get("training", {}).get(
                    "gradient_checkpointing", False
                ),
                "gradient_accumulation": self.config.get("training", {}).get(
            ...
            if (
                not self.config.get("memory", {}).get("use_compile", False)
                and self.is_apple_silicon
            ):
                results["recommendations"].append(
                    {
        ...
        if self.is_apple_silicon:
            ...
            if "model" in self.config:
                ...
                num_layers = self.config["model"].get(
                    "high_level_layers", 2
        ...
        # Log results
        logger.info(
            f"System optimization check complete with {len(results['recommendations'])} recommendations"
        )
    ...
    def generate_report(self, output_path: str = "logs/reliability_report.md"):
        ...
        if self.alert_history:
            ...
            for alert in self.alert_history[-10:]:
                ...
                report.append(
                    f"- **[{alert['level']}]** {timestamp} - {alert['message']}"
                )
        ...
        if self.training_metrics_history:
            ...
            losses = [
                m.loss for m in self.training_metrics_history if m.loss is not None
            ]
            throughputs = [
                m.throughput
                for m in self.training_metrics_history
            ...
            if losses:
                report.append(
                    f"- Loss: {np.mean(losses):.4f} (mean), {np.min(losses):.4f} (min), {np.max(losses):.4f} (max)"
                )
            if throughputs:
                report.append(
                    f"- Throughput: {np.mean(throughputs):.2f} samples/sec (mean)"
                )
        ...
        if self.system_metrics_history:
            ...
            report.append(
                f"- CPU Usage: {np.mean(cpu_percents):.1%} (mean), {np.max(cpu_percents):.1%} (max)"
            )
            report.append(
                f"- Memory Usage: {np.mean(memory_percents):.1%} (mean), {np.max(memory_percents):.1%} (max)"
            )
            ...
            if any(m.mps_memory_used is not None for m in self.system_metrics_history):
                mps_memory = [
                    ...
                    if m.mps_memory_used is not None
                ]
                report.append(
                    f"- MPS Memory: {np.mean(mps_memory):.2f} GB (mean), {np.max(mps_memory):.2f} GB (max)"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="monitor",
        choices=["monitor", "benchmark", "report"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="Alert threshold for resource usage",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report-output",
        type=str,
        default="logs/reliability_report.md",
