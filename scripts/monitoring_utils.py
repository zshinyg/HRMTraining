...
def collect_system_metrics() -> Dict[str, Union[float, int]]:
    ...
    if torch.backends.mps.is_available():
        try:
            ...
            total = torch.mps.driver_allocated_memory() if hasattr(torch.mps, 'driver_allocated_memory') else 1
...
def check_anomalies(
    ...
    has_nan = any(isinstance(v, (int, float)) and torch.isnan(torch.tensor(v)) for v in metrics.values())
    if has_nan:
    ...
    if anomalies_detected and WANDB_AVAILABLE and is_wandb_authenticated() and wandb.run is not None:
...
def init_monitoring(project_name: str = "hrm-codegen", run_name: str = "m1-27m-training") -> bool:
    """
    ...
    if WANDB_AVAILABLE and is_wandb_authenticated():
        try:
            ...
            logger.info(f"W&B monitoring initialized: project={project_name}, run={run_name}")
        ...
        logger.warning("W&B not available or not authenticated. Using local logging only.")
    
    # Log initial system metrics
    initial_metrics = collect_system_metrics()
    logger.info(f"Initial system metrics: Memory={initial_metrics['memory_usage_mb']}MB, "
...
def create_custom_charts() -> None:
    ...
    try:
        ...
        wandb.log({"memory_usage": wandb.plot.line_series(
            xs=[[0]], 
            ys=[[0]],
        ...
        wandb.log({"mps_gauge": wandb.plot.gauge(
            value=0,
            title="MPS Utilization",
            min_value=0,
            max_value=100
        )})
        
        # Create training throughput chart
        wandb.log({"throughput": wandb.plot.line_series(
            xs=[[0]],
            ys=[[0]],
            keys=["Samples/Second"],
