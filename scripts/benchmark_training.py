...
def benchmark_training(
    ...
    for batch_size in batch_sizes:
            ...
            for key, stats in timing_stats.items():
                for stat_name, value in stats.items():
                    if stat_name != "total":  # Skip total to reduce clutter
                        flat_metrics[f"benchmark/timing/{key}_{stat_name}_bs{batch_size}"] = value
        ...
        logger.info(f"  Throughput: {samples_per_second:.2f} samples/s, {tokens_per_second:.2f} tokens/s")
