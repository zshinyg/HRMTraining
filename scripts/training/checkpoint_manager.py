...
class CheckpointRotationStrategy(Enum):
    ...
    EXPONENTIAL_BACKOFF = "exponential_backoff"  # Keep checkpoints with exponential spacing
    KEEP_BEST_N = "keep_best_n"  # Keep the N best checkpoints based on a metric
    KEEP_MILESTONE = "keep_milestone"  # Keep milestone checkpoints (e.g., every 10k steps)
...
class CheckpointManager:
    ...
    def __init__(
        ...
        rotation_strategy: Union[str, CheckpointRotationStrategy] = CheckpointRotationStrategy.KEEP_LAST_N,
        ...
        if isinstance(rotation_strategy, str):
            try:
                ...
                logger.warning(f"Invalid rotation strategy: {rotation_strategy}, using KEEP_LAST_N")
        ...
        if isinstance(checkpoint_format, str):
            try:
                ...
                logger.warning(f"Invalid checkpoint format: {checkpoint_format}, using STANDARD")
        ...
        logger.info(f"Checkpoint manager initialized with output directory: {self.output_dir}")
        logger.info(f"Rotation strategy: {self.rotation_strategy.value}, Max checkpoints: {self.max_checkpoints}")
    ...
    def _load_checkpoint_history(self):
        ...
        try:
            ...
            for item in history_data:
                ...
                if metadata.is_best and (self.best_checkpoint is None or metadata.step > self.best_checkpoint.step):
                    self.best_checkpoint = metadata
                
                if self.latest_checkpoint is None or metadata.step > self.latest_checkpoint.step:
            ...
            logger.info(f"Loaded checkpoint history with {len(self.checkpoint_history)} checkpoints")
            if self.latest_checkpoint:
                logger.info(f"Latest checkpoint: step {self.latest_checkpoint.step}, epoch {self.latest_checkpoint.epoch}")
            if self.best_checkpoint:
                logger.info(f"Best checkpoint: step {self.best_checkpoint.step}, epoch {self.best_checkpoint.epoch}")
    ...
    def _save_checkpoint_history(self):
        ...
        if self.use_atomic_save:
            with tempfile.NamedTemporaryFile(mode="w", dir=self.output_dir, delete=False) as temp_file:
        ...
        try:
            ...
            if free_space_gb < self.min_free_space_gb:
                ...
                if free_space_gb < self.min_free_space_gb:
                    ...
                    logger.info(f"Successfully freed up space: {free_space_gb:.2f}GB now available")
    ...
    def _emergency_cleanup(self):
        ...
        sorted_checkpoints = sorted(
            [cp for cp in self.checkpoint_history if not cp.is_best and not cp.is_backup],
            key=lambda x: x.step
        )
    ...
    def _should_save_checkpoint(self, step: int) -> bool:
        ...
        if self.save_interval_minutes is not None:
            time_since_last = (time.time() - self.last_checkpoint_time) / 60  # Convert to minutes
    ...
    def _rotate_checkpoints(self):
        ...
        if self.rotation_strategy == CheckpointRotationStrategy.KEEP_LAST_N:
            ...
            sorted_checkpoints = sorted(regular_checkpoints, key=lambda x: x.step, reverse=True)
            ...
            to_remove = [cp for cp in sorted_checkpoints if cp.path not in to_keep_paths]
            ...
            if self.keep_best_metric:
                ...
                valid_checkpoints = [
                    cp for cp in regular_checkpoints
                    if self.keep_best_metric in cp.metrics
                ]
                
                sorted_checkpoints = sorted(
                    valid_checkpoints,
                    key=lambda x: x.metrics[self.keep_best_metric]
                )
                
                # Keep the best N checkpoints
                to_keep = sorted_checkpoints[:self.max_checkpoints]
                to_keep_paths = {cp.path for cp in to_keep}
                to_remove = [cp for cp in regular_checkpoints if cp.path not in to_keep_paths]
            else:
    ...
    def save_checkpoint(
        ...
        try:
            ...
            if self.use_atomic_save:
                ...
                with tempfile.NamedTemporaryFile(dir=self.output_dir, delete=False) as temp_file:
                    temp_path = temp_file.name
            ...
            if self.validate_on_save:
                ...
                if not is_valid:
                    ...
                    if self.latest_checkpoint and os.path.exists(self.latest_checkpoint.path):
                        logger.warning(f"Falling back to latest valid checkpoint: {self.latest_checkpoint.path}")
            ...
            logger.info(f"Saved checkpoint to {checkpoint_path} (size: {size_bytes / 1024 / 1024:.2f} MB)")
    ...
    def load_checkpoint(
        ...
        try:
            ...
            if load_model and "model" in checkpoint and self.model is not None:
                if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
                    self.model.module.load_state_dict(checkpoint["model"], strict=strict)
                else:
            ...
            if load_scheduler and "scheduler" in checkpoint and self.scheduler is not None:
    ...
    def find_closest_checkpoint(
        ...
        closest_checkpoint = min(
            self.checkpoint_history,
            key=lambda x: abs(x.step - target_step)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint-dir", type=str, required=True, help="Checkpoint directory to analyze")
    parser.add_argument("--verify", action="store_true", help="Verify all checkpoints")
    parser.add_argument("--cleanup", action="store_true", help="Clean up invalid checkpoints")
    parser.add_argument("--backup", action="store_true", help="Create backups of best and latest checkpoints")
    ...
    if args.backup:
        ...
        if manager.best_checkpoint:
            backup_path = manager.create_backup(manager.best_checkpoint, tag="best_backup")
            if backup_path:
                print(f"  Created backup of best checkpoint: {backup_path}")
        
        if manager.latest_checkpoint:
            backup_path = manager.create_backup(manager.latest_checkpoint, tag="latest_backup")
            if backup_path:
