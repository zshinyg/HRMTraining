...
class MPSOptimizer:
    ...
    def __init__(
        ...
        self.memory_fraction = max(0.1, min(0.95, memory_fraction))  # Clamp between 0.1 and 0.95
    ...
    def _check_mps_availability(self) -> bool:
        ...
        # Log availability
        if is_mps_available:
            logger.info(f"MPS is available (macOS {macos_version}, Metal {metal_version})")
    ...
    def prepare_model(self, model: Optional[nn.Module] = None) -> Tuple[nn.Module, torch.device]:
        """
        ...
        if device.type == "mps":
            ...
            logger.info(f"Model prepared for MPS training ({self.operation_mode.value} mode)")
        else:
            # Move model to CPU
            model.to(device)
        ...
        if self.stats.is_available:
            ...
            try:
                ...
                if available_mb > 100:  # Only if we have at least 100MB available
                    tensor_size = min(available_mb // 10, 100) * 1024 * 1024 // 4  # Convert MB to float32 elements
                    temp_tensor = torch.zeros(tensor_size, dtype=torch.float32, device="mps")
                    del temp_tensor
            except Exception as e:
    ...
    def get_optimal_batch_size(
        ...
        # Apply safety factor
        optimal_batch_size = max(min_batch_size, int(optimal_batch_size * safety_factor))
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        memory_utils = [entry["memory_utilization"] for entry in self._performance_history]
        throttling_events = sum(1 for entry in self._performance_history if entry["is_throttling"])
        fallback_ops = max(entry["fallback_operations_count"] for entry in self._performance_history)
        ...
        stats = {
            ...
            "throttling_percentage": throttling_events / len(self._performance_history) if self._performance_history else 0,
    ...
    def get_training_recommendations(self) -> Dict[str, Any]:
        ...
        if self._performance_history:
            ...
            throttling_percentage = sum(1 for entry in self._performance_history if entry["is_throttling"]) / len(self._performance_history)
        else:
        ...
        recommendations = {
            ...
            "operation_mode": self._recommend_operation_mode(avg_memory, max_memory, throttling_percentage),
            "batch_size_adjustment": self._recommend_batch_size_adjustment(avg_memory, max_memory),
            "mixed_precision": self._recommend_mixed_precision(),
            "gradient_checkpointing": self._recommend_gradient_checkpointing(max_memory),
            "thermal_management": self._recommend_thermal_management(throttling_percentage),
    ...
    def _recommend_operation_mode(self, avg_memory: float, max_memory: float, throttling_percentage: float) -> Dict[str, Any]:
    ...
    def _recommend_batch_size_adjustment(self, avg_memory: float, max_memory: float) -> Dict[str, Any]:
    ...
    def _recommend_thermal_management(self, throttling_percentage: float) -> Dict[str, Any]:
...
def get_mps_device_info() -> Dict[str, Any]:
    ...
    try:
        ...
        model_lines = [line for line in output.split('\n') if "Model Name" in line or "Chip" in line]
        if model_lines:
...
if __name__ == "__main__":
    ...
    if info['is_available']:
        ...
        if info['supported']:
            print("MPS is supported in your PyTorch version but not available on this system.")
