...
SAPIENT_HRM_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "external", "sapient-hrm")
)
...
def create_dummy_input(model_config):
    ...
    inputs = torch.randint(
        0, model_config.get("vocab_size", 1000), (batch_size, seq_len)
    )
...
def main():
    ...
    try:
        ...
        print("\n=== Main Configuration ===")
        print(
            f"Default architecture: {main_config.get('defaults', [{'arch': 'unknown'}])[0].get('arch')}"
        )
    ...
    if TORCH_AVAILABLE and combined_config:
        try:
            ...
            for name, info in structure.items():
                print(
                    f"{name}: {info['type']} - {info.get('parameters', 0):,} parameters"
                )
            ...
            try:
                ...
                print(
                    f"Output shapes: {[k + ': ' + str(v.shape) for k, v in outputs.items() if isinstance(v, torch.Tensor)]}"
                )
            except Exception as e:
            ...
            hrm_file_path = os.path.join(
                SAPIENT_HRM_DIR, "models", "hrm", "hrm_act_v1.py"
            )
            if os.path.exists(hrm_file_path):
                ...
                class_defs = [
                    line for line in hrm_source.split("\n") if line.startswith("class ")
                ]
...
""".format(
                hidden_size=(
                    arch_config.get("hidden_size", 512) if combined_config else "512"
                ),
