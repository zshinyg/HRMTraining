...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"hrm_hypothesis_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
def evaluate_solution(code: str, test_cases: List[str], entry_point: Optional[str] = None) -> bool:
    """
...
def generate_code_hrm(model: torch.nn.Module, prompt: str, max_tokens: int = 512) -> Tuple[str, float, float]:
    """
    ...
    try:
        ...
        start_mem = torch.cuda.max_memory_allocated() if torch.cuda.is_available() else 0
...
def generate_code_gpt2(model: torch.nn.Module, prompt: str, max_tokens: int = 512) -> Tuple[str, float, float]:
    """
...
def bootstrap_confidence_interval(results: List[bool], n_bootstrap: int = 1000, confidence: float = 0.95) -> Tuple[float, float, float]:
    """
...
def compare_models_statistical(hrm_results: List[bool], gpt2_results: List[bool], n_bootstrap: int = 1000) -> Tuple[float, float, float, float]:
    """
...
def save_results(results: Dict[str, Any], filename: str = "hypothesis_test_results.json"):
    """
...
def plot_results(results: Dict[str, Any], filename: str = "hypothesis_test_plot.png"):
    ...
    try:
        ...
        ax.bar(x - width/2, hrm_values, width, label=f'HRM ({results["hrm"]["params"]:.1f}M)')
        ax.bar(x + width/2, gpt2_values, width, label=f'GPT-2 ({results["gpt2"]["params"]:.1f}M)')
        ...
        # Add value labels
        ax.text(0, results["hrm"]["avg_time"] + 0.05, f'{results["hrm"]["avg_time"]:.2f}s', ha='center')
        ax.text(1, results["gpt2"]["avg_time"] + 0.05, f'{results["gpt2"]["avg_time"]:.2f}s', ha='center')
        ...
        # Add value labels
        ax.text(0, results["hrm"]["params"] + 2, f'{results["hrm"]["params"]:.1f}M', ha='center')
        ax.text(1, results["gpt2"]["params"] + 2, f'{results["gpt2"]["params"]:.1f}M', ha='center')
...
def run_hypothesis_test(num_samples: int = 30, max_tokens: int = 512, n_bootstrap: int = 1000):
    """
    ...
    try:
        ...
        for i, sample in enumerate(tqdm(samples, desc="Testing samples")):
            ...
            hrm_code, hrm_time, hrm_memory = generate_code_hrm(hrm_model, sample["prompt"], max_tokens)
            ...
            # Generate with GPT-2
            gpt2_code, gpt2_time, gpt2_memory = generate_code_gpt2(gpt2_model, sample["prompt"], max_tokens)
        ...
        hrm_avg_memory = np.mean(results["hrm"]["memory"]) if results["hrm"]["memory"] else 0
        gpt2_avg_memory = np.mean(results["gpt2"]["memory"]) if results["gpt2"]["memory"] else 0
        ...
        hrm_lower, hrm_upper, hrm_p = bootstrap_confidence_interval(hrm_results, n_bootstrap)
        gpt2_lower, gpt2_upper, gpt2_p = bootstrap_confidence_interval(gpt2_results, n_bootstrap)
        ...
        logger.info(f"  Parameter Efficiency: {gpt2_params/hrm_params:.1f}x fewer parameters in HRM")
        
        logger.info("\nAccuracy Metrics:")
        logger.info(f"  HRM Pass@1: {hrm_pass_at_1:.2%} ({hrm_correct}/{n_samples})")
        logger.info(f"  GPT-2 Pass@1: {gpt2_pass_at_1:.2%} ({gpt2_correct}/{n_samples})")
        ...
        if hrm_avg_memory > 0 and gpt2_avg_memory > 0:
            ...
            logger.info(f"  Memory Ratio: {gpt2_avg_memory/hrm_avg_memory:.1f}x more efficient")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--samples", type=int, default=30, help="Number of MBPP samples to test")
